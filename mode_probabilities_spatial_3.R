...
#Mike McQueen
#version 3 uses the new model and prediction function for the mnlogit package
#version 2 uses the model and prediction function for the nnet package multinom function
...
#Predict the probabilities and choice of selecting each mode based on the generated model parameters
mode_probs <- data.frame(mnlogit_predict(build6,
...
test_points_final_4326_sf <- st_transform(test_points_final_sf, 4326) %>% 
  ...
  rowwise() %>% 
  mutate(max_prob = max(p_bike, p_car, p_em))
...
interp_car <- interp2xyz(interp(x = test_points_final_4326_sf$lon,
                                ...
                                z = test_points_final_4326_sf$p_car,
                                duplicate = "mean",
                                nx = 2 * num_cells,
                                ny = num_cells),
                         data.frame = T)
...
interp_bike <- interp2xyz(interp(x = test_points_final_4326_sf$lon,
                                 y = test_points_final_4326_sf$lat,
                                 z = test_points_final_4326_sf$p_bike,
                                 duplicate = "mean",
                                 nx = 2 * num_cells,
                                 ny = num_cells),
                          data.frame = T)
...
interp_em <- interp2xyz(interp(x = test_points_final_4326_sf$lon,
                               y = test_points_final_4326_sf$lat,
                               z = test_points_final_4326_sf$p_em,
                               duplicate = "mean",
                               nx = 2 * num_cells,
                               ny = num_cells),
                        data.frame = T)
...
ggsave(final_map,
       filename = paste0("Exports/probabilities_v2_mnlogit/", map_view, test$filename, ".png"),
       width = plot_width,
       height = 4,
       units = "in",
       dpi = 450)
...
tests <- read.csv(paste0("Exports/probabilities_v2_mnlogit/", map_view, "combosets.csv"), fileEncoding="UTF-8-BOM")
